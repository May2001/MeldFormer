...
class BaseExperiment(object):
    ...
    def __init__(self, args, dataloaders=None, strategy='ddp_find_unused_parameters_false'):
    ...
    def train(self):
        self.trainer.fit(self.method, datamodule=self.data)
    ...
    def display_method_info(self, args):
        ...
        elif args.method == 'stformer':
            input_dummy = torch.ones(1, args.pre_seq_length, C, H, W).to(device)
        elif args.method == 'meldformer':
            input_dummy = torch.ones(1, args.pre_seq_length, C, H, W).to(device)
        else:
